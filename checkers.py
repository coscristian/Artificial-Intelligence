...
def get_enemy_board_pieces(board, enemy_turn):
  ...
  for i in range(len(board)):
    for j in range(len(board[i])):
        ...
        elif board[i][j] <= 2 and board[i][j] != 0:
            board_pieces.append(board[i][j])
  return board_pieces
...
def can_eat(board, x, y, enemy_turn):
  ...
  for enemy_piece in enemy_pieces:
    if x > 0 and y > 0 and enemy_piece == board[x - 1][y - 1] and board[x-1][y+1] != 0:
      return [x - 1, y - 1]
    elif x > 0 and y < len(board[x]) - 1 and enemy_piece == board[x - 1][y + 1]:
      return [x - 1, y + 1]
  return (None, None)

def eat_piece(board, x_to_eat, y_to_eat):
  print("Piece to eat: ", board[x_to_eat][y_to_eat])
  board[x_to_eat][y_to_eat] = 0

def move_to_new_position(board, x_to_move, y_to_move):
  board[x_to_move][y_to_move] = 5 #Change this piece, it must be dinamically
...
def check_diagonal_moves(x, y, board, enemy_turn):
  ...
  if x > 0: #Can move up
    ...
    if number_legal_moves == 1:
      x_legal_move = moves[0][0]
      y_legal_move = moves[0][1]
      if board[x_legal_move][y_legal_move] != 0:
        x_to_eat, y_to_eat = can_eat(board, x, y, enemy_turn)
        flag_eat = True

      x_to_move = x_legal_move
      y_to_move = y_legal_move
...
def select_coords_piece(board, enemy_turn):
  first_option = 1
  second_option = 2
  if enemy_turn:
    first_option = 3
  ...
  x, y = get_piece_position(board, first_option) 
  if x == None:
    x, y = get_piece_position(board, second_option)
  return x, y
...
def generate_legal_moves(board, player):
  ...
  x, y = select_coords_piece(board, enemy_turn)
  ...
  if enemy_turn:
      ...
      legal_moves = check_diagonal_moves(x, y, board, enemy_turn)
  print("Legal moves")
